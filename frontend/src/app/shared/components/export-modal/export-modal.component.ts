...
/// + CC field when opened for "email" rather than "download". Field-level selection isn't
/// re-picked here - it respects each field's own inline "include in export" toggle.
@Component({
...
export class ExportModalComponent implements OnChanges {
  ...
  confirm() {
    ...
    this.confirmed.emit({
      ...
      cc: this.action === 'email' && this.cc.trim() ? this.cc.trim() : undefined
    });
  }
}
