...
public class ClaudeFieldExtractionService : IAiFieldExtractionService
{
    ...
    public async Task<Dictionary<string, string>> HarmonizeFieldKeysAsync(IReadOnlyList<string> distinctKeys, CancellationToken ct = default)
    {
        ...
        var (text, error) = await CallClaudeAsync(apiKey, prompt, ct);
        ...
        {
            _logger.LogWarning(ex, "Claude field-key harmonization returned unparseable JSON; skipping harmonization for this batch");
