...
public static class AiExtractionPromptHelper
{
    ...
    private const string SignalVsNoiseInstructions = """
        ...
        - A form/table region (common on shipping, customs, and logistics documents) is sometimes extracted as ALL of its field labels printed together in one run, followed later by ALL of the corresponding values printed together in another run - not each value sitting right next to its own label. When you see a cluster of labels like this immediately followed (or later followed) by a same-sized cluster of otherwise-unlabeled values, match the Nth value to the Nth label by COUNTING POSITION in the order both were printed - do NOT match them by guessing which pairing "makes sense" (e.g. assuming Port of Discharge must equal Place of Delivery because that's common in shipping). Trust the position even when it produces a value repeating an earlier one, or a pairing that looks unusual - the position is the ground truth, a semantic assumption is a guess.
        - Paired/similarly-labeled fields (e.g. "Port of Loading" vs "Port of Discharge", "Place of Receipt" vs "Place of Delivery", "Ship From" vs "Ship To") are a common source of mixed-up extraction even outside the label/value-cluster pattern above - never copy, mirror, swap, or infer one field's value from the other just because they're the same category of information (e.g. both are ports).
        - A field's value is ONE atomic fact, never several joined together. A signature block, stamp area, or caption cluster often prints multiple short, separately-labeled pieces of text close together or on the same line (e.g. "As agent for the carrier", "As arranged", a signer's name, a carrier's name) - each is its OWN field with its OWN label, never combined with commas into one field's value just because they're visually adjacent. If a value you're about to report contains more than one distinct labeled fact strung together, split it into separate fields instead.
