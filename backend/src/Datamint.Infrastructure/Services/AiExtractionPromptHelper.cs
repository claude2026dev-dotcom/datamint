...
internal static class AiExtractionPromptHelper
{
    ...
    private const string TypeAndSectionInstructions = """
        - Classify each field with a "type" from this fixed list: "Address", "Date", "Amount", "Name", "Reference", "Contact", "Email", "URL", "Percentage", "Quantity", "Boolean", "Generic". Use "Generic" whenever a field doesn't clearly fit one of the other types - never invent a new type name. "Amount" is money/currency, "Quantity" is a plain count/measurement (no currency), "Percentage" is a value expressed with a % sign or "percent", "Boolean" is a yes/no or true/false answer, "Contact" is a phone number, "Email" is specifically an email address, "URL" is a web address/link, "Reference" is an identifying code/number that is not itself a quantity (invoice numbers, GSTIN, CIN, UDIN, PAN, account numbers).
