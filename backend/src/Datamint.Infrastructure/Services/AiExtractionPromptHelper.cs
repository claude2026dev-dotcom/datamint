...
public static class AiExtractionPromptHelper
{
    ...
    private const string SignalVsNoiseInstructions = """
        ...
        - Paired/similarly-labeled fields (e.g. "Port of Loading" vs "Port of Discharge", "Place of Receipt" vs "Place of Delivery", "Ship From" vs "Ship To") are a common source of mixed-up extraction - read each one's value strictly from the text printed next to THAT field's own label, never copy, mirror, swap, or infer one from the other just because they're the same category of information (e.g. both are ports). Two of these fields printing the identical value is unusual - re-check the source text near each label separately before reporting that.
